...
/**
 * 倒计时进度
 * https://blog.csdn.net/baidu_36619149/article/details/80163745
 */
public class CountDownProgressBar extends View {
    ...
    private void drawCircle(Canvas canvas, int center, int radius) {
        try {
            ...
            alphaAngle = currentValue * 360.0f / maxValue * 1.0f - 360.0f; // 计算每次画圆弧时扫过的角度，这里计算要注意分母要转为float类型，否则alphaAngle永远为0
    ...
    private void drawText(Canvas canvas, int center) {
        try {
            long result = ((maxValue - currentValue) * (duration / 1000L)) / maxValue; // 计算进
            String percent;
            if (0 == currentValue) {
                percent = "未开始";
