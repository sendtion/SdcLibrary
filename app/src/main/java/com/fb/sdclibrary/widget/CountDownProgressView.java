...
/**
 * 倒计时
 * https://blog.csdn.net/qq_17387361/article/details/53609301
 */
public class CountDownProgressView extends AppCompatTextView {
    ...
    public CountDownProgressView(Context context, AttributeSet attrs) {
        ...
        if (typedArray != null){
            ...
            // 园边框宽度属性
            if (typedArray.hasValue(R.styleable.CountDownProgress_circFrameWidth)){
                //有这个属性索引 获取用户设置的颜色
                circFrameWidth = typedArray.getInt(R.styleable.CountDownProgress_circFrameWidth,0);
            }else{
                //没有这个属性索引 默认是深灰色
                circFrameWidth = typedArray.getInt(R.styleable.CountDownProgress_circFrameWidth,4);
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 画文字
        int result = timeMillis * progress / 100 / 1000; // 计算进度，毫秒
        String percent;
        if (timeMillis/1000 == result) {
            percent = "未开始";
        } else if (0 == result) {
            percent = "完成"; //完成
        } else {
            //percent = (result / 60 < 10 ? "0" + result / 60 : result / 60) + ":" + (result % 60 < 10 ? "0" + result % 60 : result % 60);
            percent = (result / 3600 < 10 ? "0" + result / 3600 : result / 3600) + ":"
                    + (result % 3600 / 60 < 10 ? "0" + result % 3600 / 60 : result % 3600 / 60) + ":"
        ...
        canvas.drawText(percent, mCenterX, textY, text_paint);
        ...
        float alphaAngle = progress * 360.0f / 100 * 1.0f - 360.0f;
        canvas.drawArc(mArcRectF, 270, alphaAngle, false, mPaint); //这里的-90.是方向，大家可以改成0，90，180，-90等就可以看到效果区别
    ...
    private Runnable progressChangeTask = new Runnable() {
        ...
        public void run() {
            ...
            if (progress >= 0 && progress <= 100) {
                ...
                if ((progress == 100 || progress == 0) && mFinishListener != null){
